...
class siCommon extends mySql {
    ...
    public function insertRecords($sTable,$aFields,$aData){
        $sIsnertQuery = "INSERT INTO " . $sTable . "(" . implode(",", $aFields) . ") VALUES ";
        $sValue='';
        foreach ($aData as $aValues) {
            $sValue .= "('" . implode("','", $aValues) . "'),";
        }
        // echo $sIsnertQuery .' '. rtrim($sValue,",");
        $this->executeQuery($sIsnertQuery .' '. rtrim($sValue,","));
    }
}